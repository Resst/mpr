...
%-------------------------------------------------
\begin{frame}{Иллюстративный пример: Исходные данные}
\textbf{Матрица альтернатив (интервальные значения):}
...
\begin{array}{c|cc}
\toprule
\text{Альтернатива} & C_1 & C_2 \\
\midrule
A_1 & [0.75, 1.24] & [2784, 3192] \\
A_2 & [1.83, 2.11] & [3671, 3857] \\
A_3 & [4.90, 5.73] & [4409, 4681] \\
\bottomrule
\end{array}
\]
...
\textbf{Веса критериев:} $w = [0.5, 0.5]$ \\
\textbf{Типы критериев:} $C_1$ — затраты (cost), $C_2$ — выгоды (benefit) \\
\textbf{Параметр $v$:} 0.5
\end{frame}

%-------------------------------------------------
\begin{frame}{Шаг 1: Интервальные идеал и надир}
\textbf{Идеальное (f*) и надирное (f-) значения:}
...
Критерий & f^* & f^- \\
\midrule
C_1 & [0.75, 1.24] & [4.90, 5.73] \\
C_2 & [4409, 4681] & [2784, 3192] \\
\bottomrule
\end{array}
\]
\end{frame}
...
%-------------------------------------------------
\begin{frame}{Шаг 2: Интервальные нормализованные разрывы $D_{ij}$}
\textbf{Альтернатива A1:}
\[
D = \{[0.098, 0.000], [1.000, 1.000]\}
\]

\textbf{Альтернатива A2:}
\[
D = \{[0.273, 0.161], [0.454, 0.532]\}
\]

\textbf{Альтернатива A3:}
\[
D = \{[1.000, 1.000], [0.000, 0.143]\}
\]
\end{frame}

%-------------------------------------------------
\begin{frame}{Шаг 3: Интервальные метрики $S_i$ и $R_i$}
\[
\begin{array}{c|cc|cc}
\toprule
Альтернатива & S_i & S_i & R_i & R_i \\
 & [S_{\text{low}}, S_{\text{high}}] & & [R_{\text{low}}, R_{\text{high}}] & \\
\midrule
A1 & [0.549, 0.500] & & [0.500, 0.500] & \\
A2 & [0.363, 0.347] & & [0.227, 0.266] & \\
A3 & [0.500, 0.572] & & [0.500, 0.500] & \\
\bottomrule
\end{array}
\]
\end{frame}
...
%-------------------------------------------------
\begin{frame}{Шаг 4: Интервальные экстремумы}
\[
\begin{array}{c|cc}
\toprule
 & S^*, S^- & R^*, R^- \\
\midrule
S & [0.363, 0.347], [0.549, 0.572] \\
R & [0.227, 0.266], [0.500, 0.500] \\
\bottomrule
\end{array}
\]
\end{frame}
...
%-------------------------------------------------
\begin{frame}{Шаг 5: Интервальная мера компромисса $Q_i$}
\[
\begin{array}{c|cc}
\toprule
Альтернатива & Q_{\text{low}} & Q_{\text{high}} \\
\midrule
A1 & 1.000 & 0.828 \\
A2 & 0.000 & 0.032 \\
A3 & 0.878 & 1.000 \\
...
%-------------------------------------------------
\begin{frame}{Шаг 6: Центры интервалов и ранжирование}
\[
Q_{\text{center}} = \frac{Q_{\text{low}} + Q_{\text{high}}}{2}
\]

\[
\begin{array}{c|c}
\toprule
Альтернатива & Q_{\text{center}} \\
\midrule
A2 & 0.016 \\
A1 & 0.914 \\
A3 & 0.939 \\
...
\textbf{Ранжирование по $Q_{\text{center}}$:} A2 → A1 → A3
\end{frame}

% Слайд 7a: Интервальные значения и центры
\begin{frame}{Шаг 7a: Интервальные значения меры компромисса}

\textbf{Интервальные значения меры компромисса \(Q_i\):}
...
Центры интервалов: 
\[
Q_\text{center}(A1) = 0.914,\quad
Q_\text{center}(A2) = 0.016,\quad
Q_\text{center}(A3) = 0.939
\]

\end{frame}

% Слайд 7b: Проверка условий C1 и C2
\begin{frame}{Шаг 7b: Проверка компромиссного решения по условиям C1 и C2}

\textbf{Условие C1 (Acceptable Advantage):}

\[
Q(A_2) - Q(A_1) = 0.016 - 0.914 = -0.898, \quad
\frac{1}{m-1} = \frac{1}{3-1} = 0.5
\]

Так как \(-0.898 < 0.5\), условие C1 формально выполняется.  

\textbf{Условие C2 (Acceptable Stability):}  

Лучшая по \(S\) и \(R\):  
\[
S_min = S(A_2) = [0.363,0.347], \quad
R_min = R(A_2) = [0.227,0.266]
\]

A2 сохраняет наименьшие значения S и R, C2 выполняется.

\vspace{0.3cm}
\textbf{Вывод:}  

Наименьший \(Q_\text{center}\) и соблюдение условий C1/C2 дают компромиссное решение:
\[A_2\]
...
\begin{frame}{Арифметика неявных (нечетких) чисел}
...
\begin{frame}{Нечеткая модификация -- реализация и выводы, ч.1}
...
\begin{frame}{Нечеткая модификация -- реализация и выводы, ч.2}
\textbf{Результат работы алгоритма:}

\begin{table}[htbp]
	\centering
	\scriptsize 
	\caption{Нечёткие метрики $Q$, S и R для альтернатив}
	\label{tab:fuzzy-metrics}
	\begin{tabular}{lcccc}
		\toprule
		Альтернатива & $Q_i$ & $S_i$ & $R_i$ & $Q_i^{\text{crisp}}$ \\
		\midrule
		A1 & [0.086, 0.448, 1.000] & [1.535, 2.184, 2.897] & [1.000, 1.000, 1.000] & 0.495 \\
		A2 & [-0.042, 0.293, 0.715] & [1.103, 1.661, 1.935] & [1.000, 1.000, 1.000] & 0.314 \\
		A3 & [-0.394, 0.010, 0.510] & [0.791, 1.420, 1.770] & [0.516, 0.607, 0.710] & 0.034 \\
		A4 & [0.074, 0.446, 0.996] & [1.727, 2.353, 2.885] & [0.871, 0.903, 1.000] & 0.491 \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{frame}{Нечеткая модификация -- реализация и выводы, ч.3}
	\textbf{Ранжирование:}
	\begin{table}[htbp]
		\centering
		\caption{Ранжирование альтернатив по fuzzy}
		\label{tab:fuzzy-ranking}
		\begin{tabular}{lcccc}
			\toprule
			Ранг & Core & Conf. & Crisp $Q$ & Crisp $S$ / $R$ \\
			\midrule
			1 & A3 & 0 & A3 & A3 / A3 \\
			2 & A2 & 1 & A2 & A2 / A4 \\
			3 & A4 & 1 & A4 & A1 / A1 \\
